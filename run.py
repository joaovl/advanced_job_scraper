#!/usr/bin/env python3
"""
Master Job Scraper - Runs all scrapers with sensible defaults.

Usage:
    # Run all scrapers with defaults (London, 48h, all scrapers)
    python run.py
    ...
    # Only generate exports from existing data
...
DEFAULT_GEO_ID = "90009496"  # Greater London Area
DEFAULT_TIME_RANGE = "48h"
...
def run_linkedin_scraper(location: str, geo_id: str, time_range: str, workers: int):
    """Run LinkedIn scraper."""
    print("\n" + "=" * 70)
    print("RUNNING LINKEDIN SCRAPER")
    print("=" * 70)

    linkedin_script = SCRAPERS_DIR / "linkedin.py"
    if linkedin_script.exists():
        ...
        print(f"LinkedIn scraper not found: {linkedin_script}")
        return False


def run_workday_scrapers(location: str):
...
def run_playwright_scrapers(location: str):
...
def run_html_scrapers(location: str):
...
def load_all_jobs():
    ...
    for json_file in OUTPUT_DIR.glob("*.json"):
        # Skip master files and analysis files
        if json_file.name.startswith("master_") or json_file.name.startswith("all_jobs"):
            continue
        if "analysis" in json_file.name or "filtered" in json_file.name:
            continue
        ...
        try:
            ...
            # Handle both list format (LinkedIn) and dict format (Workday)
            if isinstance(data, list):
                # LinkedIn format: list of jobs
                company = "LinkedIn"
                scraped_at = datetime.now().isoformat()
                jobs = data
            else:
                # Workday format: dict with company, jobs, etc.
            ...
            if company not in company_files or scraped_at > company_files[company]["scraped_at"]:
                company_files[company] = {
                    ...
                    "jobs": jobs,
                    "file": json_file.name,
                    "location": data.get("location_searched", "") if isinstance(data, dict) else ""
                }
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Master Job Scraper - Run all scrapers with sensible defaults",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Run all scrapers with defaults (London, 48h)
    python run.py
    ...
    parser.add_argument("--geo-id", "-g", default=DEFAULT_GEO_ID,
                        help=f"LinkedIn geoId (default: {DEFAULT_GEO_ID} Greater London)")
    parser.add_argument("--time-range", "-t", default=DEFAULT_TIME_RANGE,
                        help=f"Time filter: 24h, 48h, 7d, 30d (default: {DEFAULT_TIME_RANGE})")
    parser.add_argument("--workers", "-w", type=int, default=DEFAULT_WORKERS,
                        help=f"Parallel workers (default: {DEFAULT_WORKERS})")

    # Scraper selection (default: run all)
    scraper_group = parser.add_argument_group("Scraper Selection (default: all)")
    scraper_group.add_argument("--linkedin-only", action="store_true",
                               help="Only run LinkedIn scraper")
    scraper_group.add_argument("--workday-only", action="store_true",
                               help="Only run Workday scrapers")
    scraper_group.add_argument("--playwright-only", action="store_true",
                               help="Only run Playwright scrapers")
    scraper_group.add_argument("--skip-linkedin", action="store_true",
                               help="Skip LinkedIn scraper")
    scraper_group.add_argument("--skip-workday", action="store_true",
                               help="Skip Workday scrapers")
    scraper_group.add_argument("--skip-playwright", action="store_true",
                               help="Skip Playwright scrapers")

    # Export options
    parser.add_argument("--export-only", action="store_true",
                        help="Only generate exports from existing data")
    parser.add_argument("--no-export", action="store_true",
                        help="Skip export generation")
    ...
    location = args.location if args.location != DEFAULT_LOCATION else scraping_config.get("location", DEFAULT_LOCATION)
    geo_id = args.geo_id if args.geo_id != DEFAULT_GEO_ID else scraping_config.get("geo_id", DEFAULT_GEO_ID)
    time_range = args.time_range if args.time_range != DEFAULT_TIME_RANGE else scraping_config.get("time_range", DEFAULT_TIME_RANGE)
    ...
    if not args.export_only:
        ...
        if args.linkedin_only:
            run_linkedin_scraper(location, geo_id, time_range, args.workers)
        elif args.workday_only:
            run_workday_scrapers(location)
        elif args.playwright_only:
            run_playwright_scrapers(location)
        else:
            # Run all scrapers (unless skipped)
    ...
    if not args.no_export:
        ...
        if json_file and excel_file:
            ...
            print(f"\nNext step: Run 'python analyze.py' to score jobs with AI")
